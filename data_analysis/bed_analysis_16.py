        ...
        for j, traj_id in enumerate(valid['trajectory_id'].unique()):
            ...
            for seg_idx, (segment_data, segment_distance) in enumerate(segments):
                ...
                if np.sum(mask) >= 2:
                    ...
                    if plot_count < 10:
                        plot_spectra(segment_distance, detrended, wavelengths_calc, pos_psd, fitted_psd, beta, residual_psd, traj_id, dataset_name, segment_number=seg_idx+1, output_path=output_paths['psd'])
                        plot_count += 1
