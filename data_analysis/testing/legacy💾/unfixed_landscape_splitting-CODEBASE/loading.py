...
from config import DATA_ROOT, RESULTS_ROOT  # noqa: E402  (path anchors; see config.py)

sys.path.insert(0, os.path.join(DATA_ROOT, 'Ockenden'))
...
# Output configuration. Results land in TESTING_LANDSCAPE_SPLITTING/ (RESULTS_ROOT), where
# the v23 test scripts expect them — not beside this file.
# (ODSA_OUTPUT_BASE env override isolates sweep runs.)
OUTPUT_BASE_PATH = os.environ.get('ODSA_OUTPUT_BASE') or os.path.join(
    RESULTS_ROOT,
    'Ockenden-regions-No_Landscape_splitting-TEST',   # ENABLE_LANDSCAPE_SPLITTING = False
    # 'Ockenden-regions-prefix-version'               # ENABLE_LANDSCAPE_SPLITTING = True
)
...
def load_datasets():
    base_path = os.path.join(DATA_ROOT, 'bedmap3_data/bedmap*/Results/')
    all_dfs = []
