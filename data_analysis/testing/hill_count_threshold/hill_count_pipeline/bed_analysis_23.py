...
from config import (ODSA_ROOT, WINDOW_SIZE, STEP_SIZE, WINDOW_TYPE, STANDARD_WINDOW,
                    peak_masking_height_threshold, bin_buffer, WINDOW_MASK,
                    HILL_BOX_M, HILL_RELIEF_THRESHOLDS,
...
def analyse_bedrock():
    ...
    base_path = os.path.join(ODSA_ROOT, 'all_data')
        ...
        for traj_id, traj_data in region_results.items():
            for w in traj_data.get('window_stats', []):
                all_window_data.append({
                    ...
                    **{f'hill_count_{t}': w.get(f'hill_count_{t}') for t in HILL_RELIEF_THRESHOLDS},
