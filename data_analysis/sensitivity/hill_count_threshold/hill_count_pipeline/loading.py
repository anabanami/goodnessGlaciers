...
from config import ODSA_ROOT, SNAPSHOT_DIR  # snapshot runs in place; data lives at the ODSA root
...
# Prefer the region definitions frozen alongside this snapshot. They set the PS71
# boxes that decide which points belong to each region, so a later edit to the live
# copy would silently change what the sweep is measured on.
sys.path.insert(0, SNAPSHOT_DIR if os.path.exists(os.path.join(SNAPSHOT_DIR, 'ockenden_coords.py'))
                else os.path.join(ODSA_ROOT, 'all_data', 'Ockenden'))
...
OUTPUT_BASE_PATH = os.environ.get('ODSA_OUTPUT_BASE') or os.path.join(
    ...
    # 'individual_region_TEST/HD_hilltest'
)
...
def load_datasets():
    base_path = os.path.join(ODSA_ROOT, 'all_data/bedmap3_data/bedmap*/Results/')
    all_dfs = []
