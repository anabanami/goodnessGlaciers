    ...
    for bundle in datasets_bundle:
        ...
        for j, traj_id in enumerate(valid['trajectory_id'].unique()):
            ...
            if ENABLE_LANDSCAPE_SPLITTING:
                ...
                # NO-split control: gap segments pass through whole, none flagged transition.
                # Mirrors the (data, distance, is_transition) contract split_by_landscape
                # already honours on its own bail-out branches.
                segments = [(sd, sdist, False) for sd, sdist in gap_segments]
