    ...
    for bundle in datasets_bundle:
        ...
        for j, traj_id in enumerate(valid['trajectory_id'].unique()):
            ...
            if segment_results:
                ...
                list_keys_collect = ['power_law_exponent', 'flow_orientation', 'hurst_exponent', 'flow_incidence_deg']
                ...
                for key in segment_results[0].keys():
                    ...
                    if key == 'elevation_min':
                        combined_stats[key] = np.min(values)
                        continue

                    if key == 'elevation_max':
                        combined_stats[key] = np.max(values)
                        continue
...
def results_summary(results):
    ...
    # 1b. Absolute Elevation
    elev_mins = [r['elevation_min'] for r in results.values() if 'elevation_min' in r]
    elev_maxs = [r['elevation_max'] for r in results.values() if 'elevation_max' in r]
    if elev_mins and elev_maxs:
        print(f"ABSOLUTE BED ELEVATION:\n  -> Min: {np.min(elev_mins):.1f}m | Max: {np.max(elev_maxs):.1f}m  (across all trajectories)")
    ...
    if all_orientations:
        ...
        # NOTE: Stratified beta analysis removed - now handled by incidence_scatter.py
        # which performs cos²θ regression on the exported segment CSV data.
        #
...
if __name__=="__main__":
    ...
    for region_name, region_results in results.items():                                       
        ...
        for traj_id, traj_data in region_results.items():
            ...
            orientations = traj_data.get('flow_orientation', [])
