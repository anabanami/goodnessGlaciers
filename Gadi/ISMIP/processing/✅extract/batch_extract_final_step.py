"""
Batch processing script for extract_final_step.py

This script is an adaptation of the original batch processor, specifically
designed to work with the `extract_final_step.py` worker. It discovers
and processes NetCDF files, generating visualizations of the final time step
for each result.

Key Features:
-   **Silent Failure Detection:** Actively checks for the creation of output
    .png files. If the worker process exits successfully but fails to produce
    any plots, it is flagged as a "Silent Failure".
...
-   **Enhanced Logging:** Provides detailed summary reports to diagnose
    any issues that arise during the batch run.

Usage:
    python batch_extract_final_step.py                    # Process all .nc files
    python batch_extract_final_step.py --pattern="*_S4_*" # Process only S4 experiments
    python batch_extract_final_step.py --parallel         # Process multiple files in parallel
    python batch_extract_final_step.py --skip-existing    # Skip files with existing output directories
    python batch_extract_final_step.py --resume           # Resume from last processed file
"""
...
def get_output_directory(nc_file):
    ...
    NOTE: This is adapted for the '_FINAL' suffix from extract_final_step.py.
    """
    return os.path.splitext(os.path.basename(nc_file))[0] + '_FINAL'
...
def process_single_file(nc_file):
    """
    Process a single NetCDF file using extract_final_step.py.
    ...
    try:
        # Run extract_final_step.py as a subprocess.
        result = subprocess.run(
            [sys.executable, "extract_final_step.py", nc_file],
            capture_output=True,
            text=True,
...
def save_resume_state(processed_files, resume_file="batch_final_resume.txt"):
    """Save the list of processed files for resume functionality."""
    with open(resume_file, 'w') as f:
        for file in processed_files:
            f.write(f"{file}\n")

def load_resume_state(resume_file="batch_final_resume.txt"):
...
def write_summary_report(results, output_file="batch_final_processing_summary.txt"):
    ...
    with open(output_file, 'w') as f:
        f.write("Batch Processing Summary (Final Step)\n")
        f.write("=" * 50 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Batch process NetCDF files with extract_final_step.py")
    ...
    for nc_file in nc_files:
        if args.resume and nc_file in processed_files:
            ...
            continue
        # --- FIX: Changed args.skip-existing to args.skip_existing ---
    ...
    # --- FIX: Changed args.dry-run to args.dry_run ---
    if args.dry_run:
    ...
    print(f"\n📝 Detailed report written to: batch_final_processing_summary.txt")
    
    if not failed and os.path.exists("batch_final_resume.txt"):
        os.remove("batch_final_resume.txt")
