...
def main():
    ...
    for data in plot_data:
        ...
        ax1.plot(x_sorted/1000, elev_detrended[sort_surf], label=data['filename'])
    ...
    for i, data in enumerate(plot_data):
        ...
        if i % 2 == 0:  # Even index (0, 2, 4...) -> plot on left axis (ax2)
            ...
            line, = ax2.plot(x_sorted/1000, vel_sorted, label=data['filename'])
    ...
    ax1.set(title='Final Surface Elevation (Detrended)', xlabel='Distance (km)', ylabel='Elevation(m)')
    ax1.legend(loc='best')
    ...
    ax2.set(title='Final Surface Velocity (Dual Axes)', xlabel='Distance (km)')
