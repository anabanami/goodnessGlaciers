...
def build_mesh_coordinates_from_netcdf(dataset, config):
    ...
    # Fallback: reconstruct mesh using same algorithm as periodic_flowline.py
    print("Reconstructing mesh coordinates using periodic_flowline.py algorithm")
...
def build_mesh_coordinates_flowline_exact(config, resolution_factor=0.5):
    """Build mesh coordinates exactly matching the periodic_flowline.py adaptive_bamg function"""
    from scipy.signal import find_peaks
    
    # Domain parameters - must match periodic_flowline.py exactly
    ...
    target_L = L_buffer_inlet + L_interest + L_buffer_terminus  # Total domain length is now 210 km
    target_nx = int(target_L * 0.01)  # Adjusted proportionally for ~100m resolution
    
    # Find optimal L that ends at a peak (matching periodic_flowline.py logic)
    search_window = 5e3
    ...
    # Match exactly the adaptive_bamg function from periodic_flowline.py
    ...
    # Use adaptive bamg meshing with exact parameters from periodic_flowline.py  
...
def build_mesh_coordinates(config, resolution_factor=0.5):
    """Build mesh coordinates from simulation parameters matching periodic_flowline.py exactly"""
...
def phase_shift_analysis(dx, bed, surface, wavelength, time_val=None):
    ...
    if time_val is not None:
        print(f"~~~Comparing bed and surface at t={time_val:.5f} years~~~")
...
def plot_signals(x, bed, surface, time_val, config, profile_id, output_dir, ylim_main=None, ylim=None):
    ...
    ax1.set_title(f'Bed vs Surface at t={time_val:.5f}yr')
    ...
    plt.tight_layout()
    fname = f"signals_t{time_val:.5f}_{profile_id:03d}.png"
    # plt.show()
...
def visualise_cross_correlation(x, bed_slope, surface_slope, wavelength, time_val, profile_id, output_dir):
    ...
    plt.title(f'Cross-correlation: bed and surface at t={time_val:.5f} years')
    ...
    plt.tight_layout()
    filename = os.path.join(output_dir, f"bed_surface_correlation_t{time_val:.5f}_{profile_id:03d}.png")
...
def visualise_phase_relationship(x, bed_slope, surface_slope, wavelength, time_val, profile_id, output_dir):
    ...
    plt.title(f'Phase relationship between bed and surface at t={time_val:.5f} years', fontsize=14)
    ...
    filename = os.path.join(output_dir, f"phase_relationship_t{time_val:.5f}_{profile_id:03d}.png")
    plt.savefig(filename, dpi=300)
    plt.close()
    return phase_deg, lag_distance
