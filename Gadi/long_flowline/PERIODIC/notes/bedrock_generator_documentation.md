...
### Profile 000 - Gaussian Bump Profile
- Straight base slope with a single, isolated Gaussian bump in the center of the undulated domain.
- Amplitude is the median of the configured amplitude range.
- The characteristic width (standard deviation) is set to 3.3 × ice thickness.
- Used as a baseline for response to isolated topographic features.

### Profile 001 - Reference Flat Profile
- Zero amplitude undulations.
- Flat slope throughout all regions.
- Used as a baseline/control case.
...
Profiles 000 and 001 receive special treatment. Profile 000 is generated with a single Gaussian bump, and Profile 001 is a flat reference case. Both have their pre- and post-extensions set to the base slope without any smoothing applied.
...
### Undulated Region (Standard Profiles)
`bed(x) = base_bed(x) + amplitude × cos(2π × (x - undulated_start) / wavelength)`

### Gaussian Bump (Profile 000)
For Profile 000, a single Gaussian bump is added to the base bed in the undulated domain:
$$ bed(x) = base\_bed(x) + A \times \exp\left(-\frac{(x' - \mu)^2}{2\sigma^2}\right) $$
Where:
- $A$ is the bump amplitude.
- $x'$ is the coordinate within the undulated domain.
- $\mu$ is the center of the undulated domain.
- $\sigma$ is the standard deviation, controlled by the `wavelength` parameter for this profile.

### Pre-extension
`bed(x) = base_bed(x) + amplitude` (for standard profiles)
`bed(x) = base_bed(x)` (for profiles 000 and 001)
