...
class BedrockGenerator:
    ...
    def smoothing(self, x, bed, wavelength, initial_elevation, profile_id=None, smoothing_length=2e3, 
        ...
        if profile_id == 1:
            # For profile 001, set both extensions to follow base slope without undulations
            pre_extension_mask = x < undulated_start
            post_extension_mask = x >= undulated_end
            
            if np.any(pre_extension_mask):
                # Pre-extension should be flat at base slope level (no amplitude)
    ...
    def generate_bedrock_profile(self, 
        ...
        with smoothing for smooth transitions (except profile 001 uses linear trend)
        """
        ...
        # Apply cosine undulation only to the undulated domain
        bed = base_bed.copy()
        ...
        # Calculate the peak height at the start of undulated region
        peak_height_at_start = initial_elevation + self.base_slope * undulated_start + amplitude
        
        # Set pre-extension to flat line at this peak height
