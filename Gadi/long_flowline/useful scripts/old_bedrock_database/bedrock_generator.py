...
class BedrockGenerator:
    ...
    def save_profile(self, x, bed, params, profile_id):
        ...
        # Add vertical line at the start of the flat extension
        plt.axvline(x=self.domain_length / 1e3, color='g', linestyle='-', alpha=0.7, label='Extension start')
    ...
    def generate_x_grid(self, extension_length=50e3):
        """Generate the x-coordinate grid with optional extension"""
        extended_domain = self.domain_length + extension_length
    ...
    def find_nearest_peak(self, x, bed, wavelength, target_x):
        """
        ...
        """
        # Only look in the original domain where undulations exist
        original_mask = x <= self.domain_length
        x_orig = x[original_mask]
        bed_orig = bed[original_mask]
        ...
        derivatives = np.gradient(bed_orig, dx)
        ...
        local_maxima = []
        for i in range(1, len(bed_orig) - 1):
            if (bed_orig[i] > bed_orig[i-1] and bed_orig[i] > bed_orig[i+1] and 
        ...
        if not local_maxima:
            ...
            return x_orig[min_deriv_idx]
        
        # Find the local maximum closest to target_x
        distances = [abs(x_orig[i] - target_x) for i in local_maxima]
        closest_peak_idx = local_maxima[np.argmin(distances)]
        
        return x_orig[closest_peak_idx]

    def smoothing(self, x, bed, wavelength, initial_elevation, profile_id=None, smoothing_length=2e3):
        """
        Apply a trend derivative smoothing starting from the nearest peak to ensure 
        smooth transition with zero derivative at the start.
        Exception: Profile 001 uses flat linear trend instead of smoothing.
        ...
        Returns:
            smoothed_bed: bedrock with smoothing applied (or linear trend for profile 001)
        """
        smoothed_bed = bed.copy()
        
        # Find the transition point (end of original domain)
        transition_x = self.domain_length
        
        # Special case for profile 001: use flat linear trend instead of smoothing
        if profile_id == 1:
            # For profile 001, transition directly to base slope without any undulations
            # from the transition point onwards
            extension_mask = x >= transition_x
            if np.any(extension_mask):
                # Set extension to follow the base slope (flat linear trend)
                extension_bed = initial_elevation + self.base_slope * x[extension_mask]
                smoothed_bed[extension_mask] = extension_bed
            return smoothed_bed
        
        # Normal smoothing for all other profiles
        # Find the nearest peak to start smoothing from
        ideal_smooth_start = transition_x - smoothing_length 
        actual_smooth_start = self.find_nearest_peak(x, bed, wavelength, ideal_smooth_start)
        
        smooth_end = x[-1]
        total_smooth_length = smooth_end - actual_smooth_start
        
        # Find indices for the smoothing zone
        smooth_mask = (x >= actual_smooth_start) & (x <= smooth_end)
        
        if not np.any(smooth_mask):
            return smoothed_bed
            
        # Get x coordinates in smoothing zone
        x_smooth = x[smooth_mask]
        
        # Normalize to [0, 1] for interpolation
        t = (x_smooth - actual_smooth_start) / total_smooth_length
        
        # Get values and derivatives at boundaries
        # Start of smoothing zone (at a peak, so derivative should be ~0)
        start_idx = np.where(x >= actual_smooth_start)[0][0]
        y0 = bed[start_idx]
        dy0_dx = 0  # Peak has zero derivative
        
        # End of smoothing zone - target the trend slope at the smoothing end
        # Calculate the bedrock incline derivative value at the peak
        y1 = initial_elevation + self.base_slope * smooth_end
        dy1_dx = self.base_slope  # Match the base slope derivative
        
        # Apply interpolation
        smoothed_values = y0 + dy1_dx * (x_smooth - actual_smooth_start)
        
        # Update the smoothed bed
        smoothed_bed[smooth_mask] = smoothed_values
    ...
    def generate_bedrock_profile(self, 
                            ...
                            extension_length=50e3,
                            profile_id=None,
                            smoothing_length=2e3):
        """
        Generate a complete bedrock profile with all features
        The original domain has undulations, followed by a flat extension
        with smoothing for smooth transition (except profile 001 uses linear trend)
        """
        # Generate the extended x grid
        x = self.generate_x_grid(extension_length)
        
        # Identify which points are in the original domain vs extension
        original_domain_mask = x <= self.domain_length
        ...
        # Apply cosine undulation only to the original domain
        bed = base_bed.copy()
        bed[original_domain_mask] += amplitude * np.cos(omega * x[original_domain_mask])
        
        # Apply transformations only to the original domain portion
        if skewness != 0:
            original_bed = bed[original_domain_mask]
            transformed_bed = self.apply_skewness(original_bed, skewness)
            bed[original_domain_mask] = transformed_bed
        
        if kurtosis != 0:
            original_bed = bed[original_domain_mask]
            transformed_bed = self.apply_kurtosis(original_bed, kurtosis)
            bed[original_domain_mask] = transformed_bed
        
        if noise_level > 0:
            original_bed = bed[original_domain_mask]
            noisy_bed = self.add_noise(original_bed, noise_level)
            bed[original_domain_mask] = noisy_bed
        
        # Apply smoothing for transition to flat extension
    ...
    def create_parameter_combinations(self, n_samples=100):
        ...
        configp_params = {
            ...
            'wavelength': 3.3 * ICE_THICKNESS, # No undulations but I need to kept a non zero wavelength
            'skewness': 0.0,
            'kurtosis': 0.0,
            'noise_level': 0.0,
            'initial_elevation': 1.0
            ...
            for wavelength in self.param_ranges['wavelength']:
                for skewness in self.param_ranges['skewness']:
                    for kurtosis in self.param_ranges['kurtosis']:
                        ...
                        if (abs(amplitude  - configp_params['amplitude']) < 1e-6 and
...
class SyntheticBedrockModelConfig:
    ...
    def init_domain_parameters(self):
        ...
        # Domain extents from the profile (now includes 15km extension)
        self.x_params = {
