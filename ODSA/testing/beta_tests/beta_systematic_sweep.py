...
Vectors are built once and re-observed per value, so the cost is one pass, not one per value.
    ...
    python beta_systematic_sweep.py                        # walks OUTPUT_BASE_PATH
    python beta_systematic_sweep.py individual_region_TEST # walk a tree of region folders
"""
import glob, os, sys
import numpy as np, pandas as pd
from pathlib import Path
HERE = Path(__file__).resolve().parent          # .../v23
ROOT_DIR = HERE.parent                          # .../ODSA
...
ROOT = sys.argv[1] if len(sys.argv) > 1 else _REGION_BASE   # run tree to read
OUT = HERE / 'beta_systematic_sweep'                         # results, never the run tree
...
if __name__ == '__main__':
    ...
    sys.stdout = Tee(str(OUT / 'beta_systematic_sweep_log.txt'))
    ...
    t.to_csv(OUT / 'beta_systematic_sweep.csv', index=False)
    pd.DataFrame(per_region).to_csv(OUT / 'beta_systematic_sweep_by_region.csv')
    print(f"\nSaved: {OUT / 'beta_systematic_sweep.csv'}")
