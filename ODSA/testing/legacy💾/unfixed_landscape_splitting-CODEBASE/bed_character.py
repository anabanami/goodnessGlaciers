...
# Processing-flag annotations live in the full codebase's config/plotting. The unfixed
# ODSA/ set (bed_analysis_22) doesn't carry them, so fall back to no-op equivalents —
# they only decorate plots/prints; bed_class classification is unaffected.
try:
    from config import PROCESSING_FLAG_NOTE as _FLAG_NOTE, processing_flag_of as region_flag
except ImportError:
    _FLAG_NOTE = {
    ...
    def region_flag(df):
        """Modal processing flag of a frame (None if the column is absent)."""
...
try:
    ...
    def _flag_suptitle(fig, title, pflag=None, **kw):
        fig.suptitle(title, **{k: v for k, v in kw.items() if k in ('fontsize', 'y', 'fontweight')})
